...
class LTR_VerticalDataset {
 public:
  ...
  void set_nfeatures(const unsigned int n) {
    nfeatures = n;
  }
  ...
  void set_ndatapoints(const unsigned int n) {
    ndps = n;
  }
  ...
  void set_nrankedlists(const unsigned int n) {
    nrankedlists = n;
  }

  ResultList get_qlist(unsigned int i) const {
    return ResultList(rloffsets[i + 1] - rloffsets[i], labels + rloffsets[i],
                      rlids[i]);
  }

  // used to find uniq values, i.e., thresholds in lmart (why not in tree).
  // used in roothistogram.
  float* get_fvector(const unsigned int i) const {
    return features[i];
  }
  void set_fvector(const unsigned int i, float* v) {
    features[i] = v;
  }
  ...
  void set_fmatrix(float** m) {
    features = m;
  }

  unsigned int get_rloffsets(unsigned int i) const {
    return rloffsets[i];
  }
  void set_rloffsets_vector(unsigned int* o) {
    rloffsets = o;
  }
  void set_rloffsets(const unsigned int i, const unsigned int o) {
    rloffsets[i] = o;
  }

  // used to find uniq values, i.e., thresholds in lmart (why not in tree)
  void sort_dpbyfeature(unsigned int i, unsigned int *&sorted,
  ...
  void set_featureid(unsigned int fidx, unsigned int val) {
    usedfid[fidx] = val;
  }
  void set_featureid_vector(unsigned int* v) {
    usedfid = v;
  }

  void set_rlids_vector(int* v) {
    rlids = v;
  }
  void set_rlids(const int i, const int v) {
    rlids[i] = v;
  }

  void set_labels_vector(double* v) {
    labels = v;
  }
  void set_label(const unsigned int i, const double v) {
    labels[i] = v;
  }
