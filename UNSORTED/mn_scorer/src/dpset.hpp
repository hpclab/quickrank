...
class dparray {
 public:
  ...
  float *get_resizedfeatures(const unsigned int i,
...
#pragma omp critical
        ...
        DataPoint *datapoint = new DataPoint(atof(token), qid, nline,
                                             maxfid + 1);
...
#pragma omp critical
      ...
      //set counters
      ndps = dparr.get_size(), nfeatures = usedfids[nfeatureids - 1] + 1;
