...
bool strisnan(const char *str) {
  return strcmp(str, "-nan") == 0 or strcmp(str, "nan") == 0
      or strcmp(str, "-inf") == 0 or strcmp(str, "inf") == 0;
}
 ...
 public:
  tnode(xmlnode &split) {
    if (split.get_nchilds("output") != 0)
      output =
          strisnan(split.get_child("output").get_text()) ?
              0.0f : atof(split.get_child("output").get_text());
    else
      featureid = atoi(split.get_child("feature").get_text()), threshold = atof(
          split.get_child("threshold").get_text());
...
class tree {
 public:
  tree(xmlnode &t) {
    id = atoi(t.get_attr("id")), weight = atof(t.get_attr("weight"));
 ...
 private:
  ...
  float weight = 0.0f;
  tnode *root = NULL;
  void get_nodeoutput(tnode const *node, float *&arr, int &size,
                      int &capacity) {
  ...
  void get_nodefeautureids(tnode const *node, unsigned int *&arr, int &size,
                           int &capacity) {
    if (node->featureid != uint_max) {
      if (size == capacity) {
        capacity = 2 * size + 1;
  ...
  void get_nodethreshold(tnode const *node, float *&arr, int &size,
                         int &capacity) {
    if (node->featureid != uint_max) {
      if (size == capacity) {
        capacity = 2 * size + 1;
...
void eval_split(xmlnode &split, unsigned int level = 0) {
  ...
  if (split.get_nchilds("output") != 0)
    printf("split pos=%s : output=%s\n", split.get_attr("pos"),
           split.get_child("output").get_text());
  else
    printf("split pos=%s : feature=%s threshold=%s\n", split.get_attr("pos"),
           split.get_child("feature").get_text(),
           split.get_child("threshold").get_text());
...
unsigned int evaldp(float *dpfeatures, unsigned int *featureidxs,
                    float *thresholds, int depth) {
...
float eval(float *fvector, unsigned int **fs, float **ts, float **os, int n,
           tree **trees, int ntrees) {
  float score = 0.0f;
...
int main(int argc, char *argv[]) {
  ...
  if (outf && ntrees > 0) {
    ...
    for (int i = 0; i < ntrees; ++i)
      fprintf(outf, " %.8f%c", trees[i]->get_weight(),
              i < ntrees - 1 ? ',' : ' ');
    fprintf(outf, "};\n\n");
    //print tree depths
    fprintf(outf, "unsigned int ds[N] = {");
    for (int i = 0; i < ntrees; ++i)
      fprintf(outf, " %d%c", trees[i]->get_depth() - 1,
              i < ntrees - 1 ? ',' : ' ');
    fprintf(outf, "};\n\n");
    //print tree leaves
