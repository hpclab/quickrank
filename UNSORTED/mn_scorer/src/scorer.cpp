...
unsigned int eval_dpontree(float *dpfeatures, unsigned int const *featureidxs,
                           float const *thresholds, const int m) {
  unsigned int leafidx = 0;
  for (int i = 0; i < M; ++i)
    leafidx |= SHL(
        dpfeatures[featureidxs[i]+FEATURE_PADDING]>thresholds[i] && i<m, M-1-i);
  return leafidx;
}
...
int main(int argc, char *argv[]) {
  ...
  //free mem
  delete[] scores, delete[] sorted_scores;
  return 0;
}
