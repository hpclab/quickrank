...
//    'xmlnode::open_xmlfile' : cannot convert parameter 2 from 'const char [5]' to 'const wchar_t *'
...
typedef struct XMLDLLENTRY xmlnode {
 private:
  ...
  /// Constructors are protected, so use instead one of: xmlnode::parseString, xmlnode::parseFile, xmlnode::open_xmlfile, xmlnode::createXMLTopNode
...
public:
  ...
  static xmlnode open_xmlfile(XMLCSTR filename, XMLCSTR tag=NULL);
  /**< The "open_xmlfile" function reports to the screen all the warnings and errors that occurred during parsing of the XML file.
   ...
   * mechanism included inside the "open_xmlfile" function).
   *
   * If the XML document is corrupted, the "open_xmlfile" method will:
   *         - display an error message on the console (or inside a messageBox for windows).
   *         - stop execution (exit).
   *
   * I strongly suggest that you write your own "open_xmlfile" method tailored to your needs. If you still want to parse
  ...
  XMLCSTR get_text(int i=0) const;///< return ith text field
  int nText() const;///< nbr of text field
  xmlnode getParentNode() const;///< return the parent node
  xmlnode get_child(int i=0) const;///< return ith child node
  xmlnode get_child(XMLCSTR name, int i) const;///< return ith child node with specific name (return an empty node if failing). If i==-1, this returns the last xmlnode with the given name.
  xmlnode get_child(XMLCSTR name, int *i=NULL) const;///< return next child node with specific name (return an empty node if failing)
  ...
  int get_nchilds(XMLCSTR name) const;///< return the number of child node with specific name
  int get_nchilds() const;///< nbr of child node
  XMLAttribute get_attr(int i=0) const;///< return ith attribute
  ...
  XMLCSTR get_attr(XMLCSTR name, int i) const;///< return ith attribute content with specific name (return a NULL if failing)
  XMLCSTR get_attr(XMLCSTR name, int *i=NULL) const;///< return next attribute content with specific name (return a NULL if failing)
   ...
   *     xmlnode::parseFile and xmlnode::open_xmlfile functions will test if the file contains ASCII
   ...
   *     xmlnode::parseFile and xmlnode::open_xmlfile functions will test if the file contains WideChar
   *     characters. If this is the case, then the file will be loaded and converted in memory to
   ...
   * file to be parsed. The xmlnode::open_xmlfile function is using this function to automatically compute
...
typedef struct XMLNodeContents {
  ...
  xmlnode child;
  XMLAttribute attrib;XMLCSTR text;
  XMLClear clear;
