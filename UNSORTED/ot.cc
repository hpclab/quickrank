...
#pragma omp parallel for
    for (unsigned int i = lbegin; i < lend; ++i) {
      ...
      node->left = nodearray[2 * i + 1] = new RTNode(lsamples, lsize, best_lvar,
                                                     lsum, lhist), node->right =
          nodearray[2 * i + 2] = new RTNode(rsamples, rsize, best_rvar, rsum,
                                            rhist), node->set_feature(
          best_featureidx, training_set->get_featureid(best_featureidx)), node
          ->threshold = best_threshold, node->deviance = minvar;
      //free mem
      if (depth) {
