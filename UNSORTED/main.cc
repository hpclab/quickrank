...
int main(int argc, char *argv[]) {
  ...
  if (argi + 6 < argc && strcasecmp(argv[argi], "lm") == 0) {
    ...
    r = new quickrank::learning::forests::LambdaMart(ntrees, shrinkage,
                                                     nthresholds, ntreeleaves,
                                                     minleafsupport, esr), ++argi;
    ...
    unsigned int minleafsupport = atoi(argv[++argi]);
    unsigned int esr = atoi(argv[++argi]);
    r = new quickrank::learning::forests::MatrixNet(ntrees, shrinkage,
  ...
  ev.evaluate(training_filename, validation_filename, test_filename,
              features_filename, output_basename);
  if (output_basename)
    ev.write();
