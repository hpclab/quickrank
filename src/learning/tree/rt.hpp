...
bool split(rtnode *node, unsigned int minls,	dpset *training_set, float *training_labels, bool require_devianceltparent=false) {
	#ifdef LOGFILE
...
class rt {
	...
	public:
		...
		void fit(histogram *hist) {
			...
			if(split(root, minls, training_set, training_labels))
			...
			while(heap.is_notempty() && (nodes==0xFFFFFFFF or taken+heap.get_size()<nodes)) {
				...
				//try split
				if(split(node, minls, training_set, training_labels)) heap.push_chidrenof(node);
