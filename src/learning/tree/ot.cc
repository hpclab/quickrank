...
#pragma omp parallel for
    for(unsigned int i=lbegin; i<lend; ++i) {
      ...
      if(depth!=treedepth-1) {
        lhist = new RTNodeHistogram(node->hist, lsamples, lsize, training_labels);
        if(node==root)
...
void ObliviousRT::fill(double **sumvar, const unsigned int nfeaturesamples, RTNodeHistogram const *hist) {
#pragma omp parallel for
  for(unsigned int f=0; f<nfeaturesamples; ++f) {
