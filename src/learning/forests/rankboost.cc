...
void Rankboost::learn(
    ...
    std::cout << "|  Weak  | Feature |   Threshold   |     R      |    alpha    |           "<<*scorer<<"           |   Time    |" << std::endl;
    std::cout << "| Ranker |    ID   |               |            |             | on training | on validation |           |" << std::endl;
    std::cout << table_hline << std::endl;
    ...
    for (unsigned int t = 0; t < T; t++) {
        ...
        printf("| %-7d", t + 1);
        std::chrono::high_resolution_clock::time_point train_t_start = std::chrono::high_resolution_clock::now();
        ...
        for (unsigned int i = 0; i < training_dataset->num_instances(); i++) {
            training_scores[i] += alpha * wr->score_document(training_dataset->at(i, 0));
        }
...
#pragma omp parallel for if(go_parallel) schedule(runtime)
    for (unsigned int q = 0; q < nq; q++) {
        std::shared_ptr<data::QueryResults> qr = training_dataset->getQueryResults(q);
        const quickrank::Label* l = qr->labels();
...
#pragma omp parallel for if(go_parallel) schedule(runtime)
    for (unsigned int f = 0; f < nf; f++) {
        ...
        for (unsigned int q = 0; q < nq; q++) {
            std::shared_ptr<data::QueryResults> r = training_dataset->getQueryResults(q);
            unsigned int nr = r->num_results();
            SDF[f][q] = new unsigned int[nr]();
            Feature* feature_values = new Feature [nr];
            for (unsigned int i = 0; i < nr; i++) {
                feature_values[i] = *training_dataset->at(training_dataset->offset(q) + i, f);
            }
    ...
    for (unsigned int q = 0; q < nq; q++) {
        ...
        for (unsigned int j = 0; j < qr->num_results() - 1; j++) {
            for (unsigned int k = j + 1; k < qr->num_results(); k++) {
                D[q][j][k] = (float) (D[q][j][k] * exp(alpha * (int) (wr->score_document(dataset->at(dataset->offset(q) + j, 0)) - wr->score_document(dataset->at(dataset->offset(q) + k, 0)))));
                z_t += D[q][j][k];
            }
        }
    }
...
#pragma omp parallel for if(go_parallel) schedule(runtime)
    ...
    std::cout << " [Done] (" << std::setprecision(5) << clean_time << " s.)" << std::endl;
}
