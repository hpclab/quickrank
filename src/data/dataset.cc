...
void Dataset::addInstance(qr::QueryID q_id, qr::Label i_label,
  ...
  if (i_features.size()>num_features_ || num_instances_==max_instances_) {
    std::cerr << "Features: " << i_features.size()
              <<" Instances: "<< num_instances_ << std::endl;
...
QueryResults Dataset::getQueryResults(unsigned int i) {
  ...
  QueryResults qr (num_results, start_label, start_data);
