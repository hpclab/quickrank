...
quickrank::metric::ir::Metric* check_and_set_metric(const po::variables_map &vm, const std::string &type)
{
...
int main(int argc, char *argv[]) {
  ...
  if (vm["algo"].as<model_string>().value == "lm")
    r = std::unique_ptr<quickrank::learning::LTR_Algorithm> (
        new quickrank::learning::forests::LambdaMart(ntrees, shrinkage, nthresholds, ntreeleaves, minleafsupport, esr) );
  ...
  std::unique_ptr<quickrank::metric::ir::Metric> training_scorer =
      std::unique_ptr<quickrank::metric::ir::Metric>( check_and_set_metric(vm, "train") );
  ...
  std::string validation_filename = check_and_set<std::string>  (vm, "valid", "Validation filename was not set.");
  ...
  quickrank::metric::Evaluator::evaluate(r.get(), training_scorer.get(), testing_scorer.get(),
                                         training_filename, validation_filename, test_filename,
                                         features_filename, model_basename);
